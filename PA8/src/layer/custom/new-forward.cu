...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k,
  ...
  definitions: y - output x - input k - kernel B - batch_size (number of images
  in x) M - number of output feature maps C - number of input feature maps H -
  input height dimension W - input width dimension K - kernel height and width
  (K x K)
  */
...
#define k4d(i3, i2, i1, i0)                                                    \
  ...
  b = blockIdx.z;
  m = blockIdx.x;
  // compute the tile width of the image - how many tiles would fit across the
  // image's width
...
__host__ void GPUInterface::conv_forward_gpu(
  ...
  conv_forward_kernel<<<gridDim, blockDim>>>(device_y, device_x, device_k, B, M,
                                             C, H, W, K);
}
...
__global__ void fc_forward_kernel(float *device_y, const float *device_x,
  ...
  HINT: the functionality of the linear layer is equivalent to the forward pass
  implemented in ./src/layer/fully_connected.cc
  */
}
